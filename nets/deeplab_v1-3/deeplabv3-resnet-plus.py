        ...
        with slim.arg_scope([slim.batch_norm], is_training=is_training):
          ...
          if aspp:
            ...
            with tf.variable_scope('aspp', [net]) as sc:
              ...
              for i in range(3):
                branch_2 = slim.conv2d(net, 256, [3,3], stride=1, rate=6*(i+1), scope='3x3conv_rate{}'.format(6*(i+1)))
              ...
              with tf.variable_scope('image-level', [net]) as sc:
                  ...
                  """
                  pooled = tf.reduce_mean(net, [1, 2], name='avg_pool', keepdims=True)
              ...
              with slim.arg_scope(resnet_arg_scope(weight_decay=args.weight_decay,
                 ...
                 with slim.arg_scope([slim.batch_norm], is_training=is_training):
                  with tf.variable_scope("low_level_features"):
                    low_level_features = end_points[scope + '/block1/unit_3/bottleneck_v1/conv1']
...
if __name__ == "__main__":
  x = tf.placeholder(tf.float32, [None, 513, 513, 3])

  net, end_points = deeplabv3(x, 21, aspp=True)
