      ...
      with slim.arg_scope([slim.conv2d, bottleneck],
        ...
        with slim.arg_scope([slim.batch_norm], is_training=is_training):
          ...
          with tf.variable_scope('aspp', [net]) as sc:
            aspp_list = []
            for i in range(4):
                branch_2 = slim.conv2d(net, num_classes, [3,3], stride=1, 
                                       rate=multi_grid[i], activation_fn=None, normalizer_fn=None,
          ...
          with tf.variable_scope('upsampling_logits',[net]) as sc:
            net = tf.image.resize_bilinear(aspp, inputs_size, name='upsample')
...
if __name__ == "__main__":
  ...
  net, end_points = deeplabv2(x, 21,depth=50)
