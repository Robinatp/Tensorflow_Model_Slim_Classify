...
class SqueezeNetTest(tf.test.TestCase):
...
#       self.assertEquals(predictions.get_shape().as_list(), [eval_batch_size])
  ...
  def testForward(self):
    ...
    inputs = tf.random_normal([1, height, width, 3])
    with slim.arg_scope(squeezenet.squeezenet_arg_scope()):
         logits, end_points= squeezenet.squeezenet(inputs,1000,compression=1.0, use_bypass=True)
