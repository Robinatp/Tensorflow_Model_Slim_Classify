...
def stem_block(inputs,
    ...
    with tf.variable_scope(scope, 'stem_block', [inputs]) as sc:
        ...
        net = slim.separable_conv2d(net, None, [3, 3], stride =1,scope="Dwconv1")
...
@slim.add_arg_scope
def transition_layer(inputs, num_channel= 128, is_avgpool=True, outputs_collections=None, scope=None):
    with tf.variable_scope(scope, 'transition', [inputs]) as sc:
#         output_channel = inputs.get_shape().as_list()[-1]
        net = slim.conv2d(inputs, num_channel, [1,1], stride=1, scope='conv')
        if is_avgpool:
...
def tiny_dsod_base(inputs,
  ...
  if len(input_shape) != 4:
    raise ValueError('Invalid input tensor rank, expected 4, was: %d' %
                         len(input_shape))
  end_points = {}
#   with tf.variable_scope(scope, 'tiny_dsod', [inputs]) as scope:
  end_points_collection = '_end_points' 
  with slim.arg_scope([slim.conv2d,slim.avg_pool2d,slim.max_pool2d,stem_block,dense_block_b,transition_layer],
                        outputs_collections = [end_points_collection]):
    with slim.arg_scope([slim.batch_norm, slim.dropout],is_training=is_training):
        stem_block_output = stem_block(inputs, num_init_channel = 64)
    
    
        blocks = [Block("stage1", dense_block_b, [{'growth_rate': 32}]*4),
                  Block("transition_layer1", transition_layer, [{"num_channel":128, "is_avgpool":True}]),
                  Block("stage2", dense_block_b, [{'growth_rate': 48}]*6),
                  Block("transition_layer2", transition_layer, [{"num_channel":128, "is_avgpool":True}]),
                  Block("stage3", dense_block_b, [{'growth_rate': 64}]*6),
                  Block("transition_layer3", transition_layer, [{"num_channel":256, "is_avgpool":False}]),
                  Block("stage4", dense_block_b, [{'growth_rate': 80}]*6),
                  Block("transition_layer4", transition_layer, [{"num_channel":128, "is_avgpool":False}])
                  ]
  ...
  with tf.variable_scope(scope, 'tiny_dsod', [inputs]) as scope:
      ...
      with slim.arg_scope([slim.conv2d,slim.avg_pool2d,slim.max_pool2d,stem_block,dense_block_b,transition_layer],
        ...
        with slim.arg_scope([slim.batch_norm, slim.dropout],is_training=is_training):
            ...
            blocks = [Block("stage1", dense_block_a, [{'growth_rate': 32}]*4),
                      Block("transition_layer1", transition_layer, [{"num_channel":128, "is_avgpool":True}]),
                      Block("stage2", dense_block_a, [{'growth_rate': 48}]*6),
                      Block("transition_layer2", transition_layer, [{"num_channel":128, "is_avgpool":True}]),
                      Block("stage3", dense_block_a, [{'growth_rate': 64}]*6),
                      Block("transition_layer3", transition_layer, [{"num_channel":256, "is_avgpool":False}]),
                      Block("stage4", dense_block_a, [{'growth_rate': 80}]*6),
                      Block("transition_layer4", transition_layer, [{"num_channel":64, "is_avgpool":False}])
                      ]
...
if __name__=='__main__':
    ...
    with slim.arg_scope(tiny_dsod_arg_scope(is_training = False)):
        logits,end_points = tiny_dsod(input_x,is_training=False)
