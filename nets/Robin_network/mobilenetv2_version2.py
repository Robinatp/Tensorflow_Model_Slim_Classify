...
InvertedResidual = namedtuple('InvertedResidual', ['kernel', 'depth', 'num', 'stride','t']) # t is the expension factor
_CONV_DEFS = [
    Conv(kernel=[3, 3], stride=2, depth=32),
    InvertedResidual(kernel=[3, 3], depth=16,  num=1, stride=1, t=1),
    InvertedResidual(kernel=[3, 3], depth=24,  num=2, stride=2, t=6),
    InvertedResidual(kernel=[3, 3], depth=32,  num=3, stride=2, t=6),
    InvertedResidual(kernel=[3, 3], depth=64,  num=4, stride=2, t=6),
    InvertedResidual(kernel=[3, 3], depth=96,  num=3, stride=1, t=6),
    InvertedResidual(kernel=[3, 3], depth=160, num=3, stride=2, t=6),
    InvertedResidual(kernel=[3, 3], depth=320, num=1, stride=1, t=6),
...
def _inverted_residual_bottleneck(inputs, depth, stride, expand_ratio, scope=None):
  with tf.variable_scope(scope, 'InvertedResidual', [inputs]) as sc:
    ...
    print(depth_in)
    if expand_ratio == 1:#first bottleneck only include depth-separable convolution when expand_ratio=1
        output = slim.separable_conv2d(inputs, None, 3, depth_multiplier=1, stride=stride,
                              activation_fn=tf.nn.relu6, normalizer_fn=slim.batch_norm, scope='depthwise')
        output = slim.conv2d(output, num_outputs=depth, kernel_size=[1,1], stride=1,
                                  activation_fn=None, normalizer_fn=slim.batch_norm, scope='project')
        return output
        
    output = slim.conv2d(inputs, num_outputs=expand_ratio*inputs.get_shape().as_list()[-1], kernel_size=[1,1], stride=1,
                              activation_fn=tf.nn.relu6, normalizer_fn=slim.batch_norm, scope='expand')
    output = slim.separable_conv2d(output, None, 3, depth_multiplier=1, stride=stride,
                              activation_fn=tf.nn.relu6, normalizer_fn=slim.batch_norm, scope='depthwise')
    output = slim.conv2d(output, num_outputs=depth, kernel_size=[1,1], stride=1,
                              activation_fn=None, normalizer_fn=slim.batch_norm, scope='project')

    #when stride 1 and the same depth of input and output, there is a residual,all others directly return
  ...
  with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding='SAME'):
      ...
      for i, conv_def in enumerate(conv_defs):
        ...
        if isinstance(conv_def, Conv):
          ...
          net = slim.conv2d(net, num_outputs=depth(conv_def.depth), kernel_size=conv_def.kernel,
...
if __name__ == "__main__":
    ...
    with slim.arg_scope(mobilenet_v2_arg_scope(is_training=False)):
        logits, end_points= mobilenet_v2(inputs,100)   
