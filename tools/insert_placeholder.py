...
input_checkpoint = '../tmp/checkpoints/vgg_16.ckpt'

# Where to save the modified graph
save_path = '../tmp/checkpoints/with_placeholder'

# TODO(shizehao): use graph editor library insead
with tf.Graph().as_default() as graph:
  input_images = tf.placeholder(tf.float32, [None, 224, 224, 3], 'input')
  with slim.arg_scope(vgg.vgg_arg_scope()):
    logits, _ = vgg.vgg_16(input_images,
                            num_classes=1000,
                            is_training=False)
